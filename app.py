...
def fetch_latest_snapshot_date(title_num="1"):
    ...
    data = resp.json()  # This is a dict with 'content_versions' and 'meta'

    # If data doesn't have 'meta' or 'latest_issue_date', handle that
    if "meta" not in data or "latest_issue_date" not in data["meta"]:
        # Means we got an unexpected JSON structure or empty
        return None

    # That "latest_issue_date" is your best guess for the newest snapshot date:
    return data["meta"]["latest_issue_date"]
# ----------------------------------
...
def latest_analyze():
    ...
    try:
        ...
        if not latest_date:
            # Means the meta object didn't have latest_issue_date or
            # the endpoint returned an unexpected structure.
            return jsonify({
                "success": False,
                "error": f"No snapshot date found for title {title_num}"
            }), 404

        # Now use `latest_date` with `fetch_ecfr_xml(...)`
        # or whatever step you do next.
